...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  const applyTimeMode = useCallback((mode: TimeMode) => {
    ...
    if (typeof document !== 'undefined') {
      ...
      // Backward compatibility for legacy styling
  ...
  useEffect(() => {
    ...
    // Interval: Auto check every 60 seconds to transition seamlessly as time progresses
  ...
  const cycleTimeMode = () => {
    ...
    if (isAuto) {
      // From Auto -> jump to next manual mode
      const nextIndex = (currentIndex + 1) % modes.length;
      setTimeMode(modes[nextIndex]);
    } else {
      if (currentIndex === modes.length - 1) {
        // From last mode -> return to Auto
        resetToAuto();
      } else {
